...
async function get_id(num) {
	const reponse = await fetch(API + "all?fields=flags,languages,population,region,subregion,name,capital,currencies");
	const data = await reponse.json();
	return data[num];
}

function actualise(alt_srcs, texts, title) {
...
document.addEventListener("DOMContentLoaded", async () => {
	...
	const data = await get_id(await get_info_country());
	...
	actualise(alt_srcs, texts, data.name.common);
});
